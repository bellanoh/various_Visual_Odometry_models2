...
# Custom model
# =========================================
# 비디오 확장 (num_frames, attention_type): EfficientViT를 비디오 wrapper로 (EfficientViT 특징 + time attention)
class VideoEfficientViT(torch.nn.Module):
    def __init__(self, base_model, num_frames, attention_type='divided_space_time', attn_dropout=0.1, dim_head=64,
        ...
        # self.feature_extractor = lambda x: torch.nn.functional.adaptive_avg_pool2d(
        #     self.base_model.forward_features(x)[-1], 1
        # ).flatten(1)
        self.feature_extractor = lambda x: torch.nn.functional.adaptive_avg_pool2d(
            self.base_model.forward_features(x)[-1] if isinstance(self.base_model.forward_features(x),
                                                                  (list, tuple)) else self.base_model.forward_features(
                x),
            (1, 1)
        ).flatten(1)
        embed_dim = base_model.num_features  # EfficientViT embed dim (e.g., 384 for B2)

        # Time attention 추가 (ViT-like: MultiheadAttention)
        self.time_attn = torch.nn.MultiheadAttention(embed_dim=embed_dim, num_heads=embed_dim // dim_head, dropout=attn_dropout,)
                                                     #kdim=dim_head, vdim=dim_head)
        self.custom_fc = torch.nn.Linear(embed_dim, 3)
    ...
    def forward(self, x):
        ...
        feats = self.feature_extractor(x)  # (B*T, 384)
        ...
        drop_rate = getattr(self.base_model.head, 'drop_rate', 0.0)  # .drop_rate 속성 확인, 없으면 0.0
        if drop_rate > 0.0:
            feats = torch.nn.functional.dropout(feats, p=drop_rate, training=self.training)

        out = self.custom_fc(feats)# fc만 적용: (B, in_features) -> (B, num_classes)
        return out
