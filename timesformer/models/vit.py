...
class VisionTransformer(nn.Module):
    ...
    #         x)  # x.shape = (B, total_tokens, embed_dim), total_tokens = num_frames * num_patches + 1 (CLS 포함)
    #
    #     B = x.size(0)
    #     num_patches_per_frame = (self.img_size[0] // self.patch_size) * (
    #                 self.img_size[1] // self.patch_size)  # e.g., 196
    #     if isinstance(num_patches_per_frame, tuple):
    #         num_patches_per_frame = num_patches_per_frame[0] * num_patches_per_frame[1]
    #
    #     if isinstance(self.num_frames, tuple):
    #         self.num_frames = self.num_frames[0]
    #
    #     total_patches = int(self.num_frames) * num_patches_per_frame  # e.g., 20 * 196 = 3920
    #     total_tokens = total_patches + 1  # CLS 포함, e.g., 3921
    #
    #     # CLS를 제거하지 않고 전체를 reshaping. 하지만 view(B, T, N, D)에서 N을 CLS 때문에 조정하기 어려우니,
    #     # CLS를 별도로 추출하고 나머지 패치만 reshaping 후 head에 넘김 (CLS 유지 목적 달성 위해 head에서 CLS 활용 가능하게)
    #     cls_token = x[:, 0:1, :]  # CLS 토큰 추출 (B, 1, embed_dim), 제거 안 하고 유지
    #     patches = x[:, 1:, :]  # 패치만 (B, total_patches, embed_dim)
    #
    #     # 패치 reshaping
    #     patches = patches.view(B, int(self.num_frames), num_patches_per_frame,
    #                            self.embed_dim)  # (B, T=20, N=196, D=384)
    #
    #     # head에 CLS와 patches를 함께 넘김 (head가 이를 처리하도록 가정. 만약 head가 평균 풀링 등이라면 여기서 CLS 활용)
    #     # 예: head가 classification head라면, CLS를 global representation으로 사용하거나 patches와 결합
    #     x = torch.cat([cls_token.unsqueeze(1).repeat(1, int(self.num_frames), 1, 1), patches],
    #                             dim=2)  # 예시: CLS를 각 frame에 반복 붙여 (B, T, 1 + N, D) 형태로 head에 넘김
    #     # 토큰 차원(dim=2)을 pooling해서 (B, T, D)로 줄임
    #     x = x.mean(dim=2)  # 또는 torch.max(x, dim=2).values, 또는 self.pool_layer(x) 등 원하는 pooling
    #     # 이제 x: (B, T, D=384)
    #
    #     x = self.head(x)  # head가 nn.Linear(D, num_classes=3)이면 (B, T, 3)
    #     return x

    def forward(self, x):
        x = self.forward_features(x)  # [B, 3921, embed_dim]
        x = torch.mean(x, dim=1)  # 토큰 차원 평균 → [B, embed_dim]
        x = self.head(x)  # [B, 3]
        return x
